const API_KEY = 'AIzaSyAbmDQHNM_PRyZMdju71yTwDQ0b90kev70'; 
...
// Your existing Gemini code remains...
...
document.getElementById('translate-toggle').addEventListener('click', () => {
  const allElements = document.querySelectorAll('h1, button, input::placeholder');

  allElements.forEach(el => {
    if (el.textContent.includes('বাংলা')) {
      el.textContent = el.textContent.replace('বাংলা', 'English');
      document.querySelector('h1').textContent = 'অনলাইন ফার্স্ট এইড চ্যাটবট';
      document.getElementById('user-input').placeholder = 'আপনার বার্তা লিখুন...';
    } else {
      el.textContent = el.textContent.replace('English', 'বাংলা');
      document.querySelector('h1').textContent = 'Online First Aid Chatbot';
...
document.getElementById('media-input').addEventListener('change', (e) => {
  ...
  reader.onload = () => {
    ...
    const mediaContent = document.createElement(file.type.startsWith('video') ? 'video' : 'img');
    mediaContent.src = reader.result;
    mediaContent.controls = file.type.startsWith('video');
    mediaContent.style.maxWidth = '200px';
